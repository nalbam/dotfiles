...
Analyze entire codebase and documentation, find gaps, update docs. 문서 갱신도 AGENTS.md 의 Surgical Changes 를 따른다 — 요청 외 인접 문서를 임의로 손보지 않는다.
...
- Only `README.md` and `AGENTS.md` in project root
...
스캔 시 dependency / build / cache / VCS / IDE / 테스트 산출물 등은 제외한다. 전체 목록은 `code-audit` 스킬 (`~/.agents/skills/code-audit/SKILL.md`) 의 *Exclude Patterns* 표를 *유일한 source* 로 한다.
...
├── AGENTS.md         # AI instructions only
└── docs/
    ├── README.md     # Index
    └── *.md          # All other docs
```
