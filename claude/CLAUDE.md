...
**Always respond in Korean (한국어로 응답하라).** 대화(응답)는 존댓말, 문서는 해라체 로 쓴다. 코드·명령어·파일명·API 이름·기술 용어는 원문(영어) 유지. 간결하고 명확하게 — 장황한 인사·요약 생략.
...
- **Solve the right problem** — 요청한 문제만 정확히, 스코프 크리프·불필요한 변경 회피. 부수 개선은 별도 PR.
