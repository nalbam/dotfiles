...
5. **Core Principles** — 코드 품질·안전 원칙
6. **Claude Code Usage** — 도구 사용 규약
...
- 코드, 명령어, 기술 용어는 영어 유지
- 커밋 메시지, 코드 주석은 영어 유지
- PR 제목·본문, 이슈 코멘트는 한국어 (단, 커밋 메시지와 코드 주석은 영어)
- 간결하고 명확한 표현 선호
...
- `/commit`, `/commit-push`, `/pr-create` 등 슬래시 커맨드는 명시적 커밋·푸시 지시로 간주
...
- 기존 유사 코드·유틸리티를 먼저 탐색하고 패턴을 재사용 (중복 구현 금지)
- 기존 파일 수정을 우선. 새 파일·문서는 꼭 필요할 때만 생성
...
- **Ask when uncertain** — 불확실하면 추측 대신 질문
- **Do only what's needed** — 필요한 일만 수행, 불필요하면 멈춤
...
- 신규 코드는 테스트 동반. 커버리지 측정 가능 시 80%+ 목표
- 버그 수정에는 회귀 테스트 필수
- 테스트 간 상태 공유 금지
