...
행동 원칙: 문서 갱신도 *외과적 변경* 원칙 — 요청 외 문서를 임의로 손보지 않는다 (`skills/coding-style/SKILL.md#surgical-changes--외과적-변경`). 새 파일은 *명확한 필요*가 있을 때만 생성. *프로젝트 관례 우선* — 기존 문서 구조(예: `docs/`, Sphinx 의 `docs/source/`, README 만 사용 등)를 따른다.
