...
머지된 로컬·원격 브랜치를 식별하여 *사용자 확인 후* 삭제한다. git 안전 규칙은 `rules/git-workflow.md` 가 source — 브랜치 삭제는 *파괴적 작업* 으로 사용자 명시 요청 시에만 수행한다.
...
| Rule | 이유 |
|------|------|
| 사용자 확인 없이 삭제 금지 | 파괴적 작업 (`rules/git-workflow.md`) |
