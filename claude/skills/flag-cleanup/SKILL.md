...
활성/비활성이 결정된 feature flag·gate·experiment·staged rollout을 식별하고 *안전한 제거 PR*을 만든다. 변경 작업은 `rules/coding-style.md#surgical-changes--외과적-변경` 을 따른다 — flag 정리 PR에는 *flag 정리 외* 변경이 섞이지 않는다.

## Philosophy

- **잊혀진 flag 는 영원한 부채** — 정리 안 하면 분기·경로·테스트가 누적
- **flag 제거는 행동 보존** — 현재 *실제 동작* 분기만 남기고 반대 분기를 제거 (동작 변경 X)
- **사용자가 결정** — LLM 은 *후보를 식별*만 한다. 어느 flag 를 제거할지·어느 분기가 활성인지는 사용자 확인
...
LLM 은 *어느 flag 를 정리할지 결정하지 않는다*. 사용자에게 식별 결과를 표로 제시하고 확인 받는다:
...
- 무관한 인접 코드는 *손대지 않는다* (`rules/coding-style.md#surgical-changes--외과적-변경`)
...
| Rule | 이유 |
|------|------|
| 사용자 확인 없이 flag 제거 금지 | LLM 이 활성 분기를 잘못 추정할 수 있음 |
| flag 1개 = PR 1개 | 롤백 단위 보존 |
| 분석 후 확신 없으면 사용자에게 환원 | "어느 분기가 활성?"이 불명확하면 멈춤 |
| 외과적 변경 — 무관한 리팩토링 금지 | `rules/coding-style.md#surgical-changes--외과적-변경` |
