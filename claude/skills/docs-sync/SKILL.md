...
- **대상**: 루트 안내 문서 (`README.md`·`CLAUDE.md`·`CONTRIBUTING.md` 등), `docs/` 전체
...
**이미 승인된 문서 수정 범위는 바로 진행한다.** 사용자가 점검·보고만 요청했거나 삭제·새 문서 추가가 요청 범위를 넘어서는 경우에만 범위와 근거를 제시하고 확인한다.
...
**종료 조건**: 승인된 범위의 부정확한 서술과 끊어진 참조를 교정하고 검증한다. 범위 밖 항목은 근거와 함께 별도로 보고한다.
