...
- 시니어 엔지니어처럼 생각하세요.
- 추측으로 뛰어들거나 성급히 결론내리지 마세요.
- 항상 여러 접근을 평가하고, 결정하기 전에 최소 두 가지 대안을 비교하세요.
- 문제 정의 → 작고 안전한 변경 → 변경 리뷰 → 리팩터링 — 이 루프를 반복하세요.

## 코드 변경 전 확인사항

- 호출/참조 경로를 포함하여 관련 파일을 처음부터 끝까지 읽으세요.
- 정의, 참조, 호출 지점, 관련 테스트, 문서/설정을 찾아 읽으세요.
...
- **파일 크기 제한 유지**: 가독성, 유지보수성, 협업을 개선하기 위해 파일을 700줄 미만으로 유지하세요. (세부 내용은 '파일 크기 가이드라인' 참조)
...
- **최대 파일 길이**: 모든 소스 코드 파일을 700줄 미만으로 제한합니다.
...
- 전체 문맥을 읽지 않고 코드를 수정하지 마세요.
- 코드, 로그, 커밋에 비밀값을 노출하지 마세요.
- 실패나 경고를 무시하지 마세요.
- 근거 없는 최적화나 추상화를 도입하지 마세요.
- 광범위한 예외를 남용하지 마세요.
